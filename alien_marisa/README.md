...
8. `/alienmarisa set <字段名> <值>` —— 运行时改配置，例：`set REPLAY_DAMAGE true`、`set PROJECTILE_SPEED_CAP 3`
   （可用字段名见 `/alienmarisa config`；调妖归弹幕的预判准度用 `set NO_GRAVITY_PREFIX ...`）

> ⚠️ **自测必踩的坑**：复现弹幕时会 `setOwner(caster)`，而测试期 caster 就是你自己 ——
> 玩家免疫自己的投射物，所以「她用我的招回敬我」打不到自己身上。
> 请用 `/alienmarisa last <另一个实体>`，或先 `/summon minecraft:zombie` 几个靶子。
> 近战复现不走投射物，没这个问题。

> 默认 `REPLAY_DAMAGE=false`：复现出的弹幕不造成伤害，只验证弹道与演出。
> 想验证杀伤：`/alienmarisa set REPLAY_DAMAGE true`。

**编译后第一次进游戏，先看启动日志这一行**（确认反射探到了正确的方法）：

```
[AlienMarisa] NBT 桥接: capture=serializeNBT(provider)
```

若显示 `capture=NONE`，说明当前 MC 版本的方法名不在探测列表里，
把 `AlienConfig.DEBUG` 打开能看到具体异常 —— 这种情况下妖归弹幕会降级为「只抄类型 + 弹道」。
...
- **弹道预测按类型取重力**：录制时就把「这发弹幕吃不吃重力」定死写进能力里
  （`entity.isNoGravity()` 或命中 `NO_GRAVITY_PREFIX` 白名单），复现时直接读。
  **不用弹速去猜** —— 妖归的高速弹幕会被误判成箭矢，落点整体算偏。
- **NBT 快照只在「确认是新招」之后才抓**：抓快照要反射 + 深拷贝 + 估体积，
  妖归弹幕每秒上百发，先做无快照探针查库，老招一律不抓。
  距离粗筛也放在事件最前，远处弹幕不进后续逻辑。
...
| 能力库 | 进程内静态库，**服务端重启即全忘**。`saveAll()` / `loadAll()` 已写好但无人调用，要接 Boss 实体时挂上去 |
| `OBSERVE_RANGED` | 默认 **false**。射一次弓会连产两条记录（RANGED 无弹道、复现只能放粒子，是废记录；PROJECTILE 才有真实弹道），默认只留后者 |
| 播报 | 同一玩家 `ANNOUNCE_COOLDOWN`（默认 60 tick）内只播一次，防止连射跨档刷屏 |
